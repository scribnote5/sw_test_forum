<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="C# Coding Convention 가이드라인" :subPage="styleCopRule" :paths="['C# Coding Convention', 'C# Coding Convention 가이드라인 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">StyleCop 가이드라인 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-guidelines/form/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-guidelines/" + idx,
          {
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-guidelines/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("styleCopGuidelineIdx", idx);
      ...
      localStorage.setItem("result", "/style-cop-guideline/update-success");
      router.push("/style-cop-guideline/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-guidelines/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/style-cop-guideline/list/" + styleCopIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      styleCopIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
      styleCopRule,
