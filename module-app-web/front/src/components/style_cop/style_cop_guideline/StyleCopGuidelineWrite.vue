<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="C# Coding Convention 가이드라인" :subPage="styleCopRule" :paths="['C# Coding Convention', 'C# Coding Convention 가이드라인 작성']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">StyleCop 가이드라인 정보</th>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="overflow: visible">
              <input type="text" name="projectName" v-model="projectName" class="form-control" placeholder="한화시스템 통합랩 - 함정 통합기관제어체계">
          ...
          <tr>
            ...
            <td>
              ...
              <input type="text" name="guidelineResultNote" v-model="guidelineResultNote" class="form-control" placeholder="STATIC 4.4.2 패치 예정">
          ...
          <tr>
            <td colspan="2">
              <CodeMirror pageInformation="write" mode="text/x-csrc"></CodeMirror>
            </td>
          </tr>
          <tr>
            <td colspan="2">
      ...
      <div class="d-flex justify-content-between mx-3 my-5">
        <div class="d-flex">
          <router-link :to="'/style-cop-guideline/list/' + styleCopIdx">
...
export default {
  ...
  setup() {
    ...
    // variable
    const styleCopIdx = ref(route.params.styleCopIdx);
    const title = ref("");
    const hashTags = ref("");
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-guidelines/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-guidelines",
          {
          ...
          .then((response) => {
            styleCopGuidelineIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("styleCopGuidelineIdx", styleCopGuidelineIdx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-guidelines/attached-file",
            formData, {
              headers: {
            ...
            .catch((error) => {
              ...
              deletePost(styleCopGuidelineIdx);
              isExit = true;
            })
            .then(() => {
              return isExit;
      ...
      localStorage.setItem("result", "/style-cop-guideline/write-success");
      router.push("/style-cop-guideline/read/from-rule-list/" + styleCopGuidelineIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      styleCopIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
      styleCopRule,
