<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="C# Coding Convention 규칙" :paths="['C# Coding Convention', 'C# Coding Convention 규칙 리스트']" title=""/>
    ...
    <div class="container-fluid">
      ...
      <div class="page-content">
        <table class="table table-hover mobile-table-list mt-4">
          ...
          <tr class="d-none d-lg-table-row">
            ...
            <th style="width: 50%" class="text-center">규칙</th>
          ...
          <tbody>
          <!-- styleCopListByPriority -->
          <tr v-for="(styleCop, i) in styleCopListByPriority" :key="i">
            <!-- Desktop 번호 -->
          ...
          <!-- styleCopList -->
          <tr v-for="(styleCop, i) in styleCopList.content" :key="i">
            <!-- Desktop 번호 -->
            <td class="d-none d-lg-table-cell text-center">{{ styleCopList.totalElements - styleCopList.pageable.offset - i }}</td>
            <td>
              <!-- Mobile -->
          ...
          <tr v-if="styleCopList.content.length === 0">
      ...
      <!-- Pagination -->
      <Pagination :first="styleCopList.first" :last="styleCopList.last" :totalPages="styleCopList.totalPages"
                  :number="styleCopList.number" :startNumber="startNumber" :endNumber="endNumber"
                  :searchList="searchList"/>

      <div class="d-flex justify-content-end mx-4 mb-4" v-if="access">
        <router-link :to="'/style-cop/write'">
  ...
  setup() {
    ...
    const searchList = async (pageParam) => {
      ...
      if (pageParam.page === 1 && isEmpty(searchKeyword.value)) {
        await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop/high-priority-list",
            {},
        )
            .then((response) => {
              styleCopListByPriority.value = response.data;
              // dayjs
              for (const styleCop of styleCopListByPriority.value) {
        ...
        styleCopListByPriority.value.length = 0;
      }
      ...
      const uri = createUri(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop/list", params);

      await axios.get(uri,
          {},
      )
          .then((response) => {
            // styleCopList.value.content.length = 0;
            styleCopList.value = response.data;
            startNumber.value = Math.floor(styleCopList.value.number / 10) * 10 + 1;
            endNumber.value = (styleCopList.value.totalPages > startNumber.value + 9) ? startNumber.value + 9 : (styleCopList.value.totalPages == 0 ? 1 : styleCopList.value.totalPages);

            // dayjs
            for (const styleCop of styleCopList.value.content) {
              styleCop.createdDate = dayjs(styleCop.createdDate).format("YYYY.MM.DD.");
            }
          })
          .catch((error) => {
    ...
    return {
      // variable
      styleCopListByPriority, styleCopList, searchType, searchKeyword, startNumber, endNumber, access,
