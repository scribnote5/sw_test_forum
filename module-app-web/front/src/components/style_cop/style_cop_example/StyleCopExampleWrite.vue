<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="C# Coding Convention 예제 코드" :subPage="styleCopRule" :paths="['C# Coding Convention', 'C# Coding Convention 예제 코드 작성']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-examples/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
          ...
          .catch((error) => {
            ...
            router.push("/style-cop-example/list/");
          })
          .then(() => {
          });

      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-examples/priority-list-write/" + styleCopIdx.value,
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-examples",
          {
          ...
          .then((response) => {
            styleCopExampleIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      localStorage.setItem("result", "/style-cop-example/write-success");
      router.push("/style-cop-example/read/from-rule-list/" + styleCopExampleIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      styleCopIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
