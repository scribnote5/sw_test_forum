<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="C# Coding Convention 예제 코드" :subPage="styleCopRule" :paths="['C# Coding Convention', 'C# Coding Convention 예제 코드 보기']" :title="styleCopExample.title"/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tr class="mobile-only-visible d-md-table-row d-lg-none">
            <td colspan="2">
              <strong>StyleCop 규칙: </strong> {{ styleCopRule }} <br>
              <strong>우선순위: </strong>
              <Priority pageInformation="read" :priority="styleCopExample.priority" :maxPriority=Number(4)></Priority>
      ...
      <Comment path="style-cop-example-comments" idxName="styleCopExampleIdx" :idx="styleCopExample.idx" :commentList="styleCopExample.commentDtoList"></Comment>

      <div class="d-flex justify-content-between flex-column flex-md-row mx-3 my-5">
        <div class="d-flex">
          <router-link :to="this.$route.meta.fromRuleList ? '/style-cop-example/list/' + styleCopExample.styleCopIdx : '/style-cop-example/list'" class="me-2">
            <button class="btn btn-main-grey d-flex align-items-center">목록<img :src="require(`@/assets/images/list-white.svg`)" class="ms-2"></button>
          </router-link>
          <router-link :to="'/style-cop/read/' + styleCopExample.styleCopIdx" class="ms-2">
        ...
        <div class="d-flex mt-2 mt-md-0" v-if="styleCopExample.access">
          <router-link :to="'/style-cop-example/update/' + idx" class="me-2">
...
export default {
  ...
  setup() {
    ...
    onBeforeMount(async () => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-examples/read/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop/style-cop-rule/" + styleCopExample.value.styleCopIdx,
          {},
      )
          .then((response) => {
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/style-cop-examples/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/style-cop-example/list/" + styleCopExample.value.styleCopIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      // variable
      styleCopExample, idx, compliantExample, nonCompliantExample, badCasePositionList, goodCasePositionList, styleCopRule,
