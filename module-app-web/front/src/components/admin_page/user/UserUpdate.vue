...
// utils
import {deleteArrayIndexIsEmpty, parseErrorMsg, validateContact, validateEditor, validateEmail, validateLength, validateLengthAndIsEmpty, validatePassword} from "@/utils/validation-util";
...
export default {
  ...
  setup() {
    ...
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/users/form/" + idx,
          ...
          .then((response) => {
            ...
            // autoComplete 설정
            createAutoComplete(response.data.autoCompleteTitle, "department", department);
    ...
    const updatePost = async () => {
      ...
      if (!(validatePassword(true, 'password', 'checkPassword', password.value, checkPassword.value, true)
    ...
    return {
      ...
      // function
      validateEditor, validatePassword, validateEmail, validateContact,
