<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="Toolchain 트러블슈팅" :paths="['Toolchain', 'Toolchain 트러블슈팅 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">Toolchain 트러블슈팅 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/toolchains/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/toolchains/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/toolchains/" + idx,
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/toolchains/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("toolchainIdx", idx);
      ...
      localStorage.setItem("result", "/toolchain/update-success");
      router.push("/toolchain/read/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/toolchains/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/toolchain/list/" + misraCIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      title, priority, hashTags, toolCategory, ideName, compilerName, compilerNote,
      priorityArray,
