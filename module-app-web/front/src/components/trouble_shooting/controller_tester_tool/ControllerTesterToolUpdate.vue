<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="Controller Tester 트러블슈팅" :paths="['Controller Tester', 'Controller Tester 트러블슈팅 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">Controller Tester 트러블슈팅 정보</th>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="overflow: visible">
              <input type="text" name="toolName" v-model="toolName" class="form-control" placeholder="Controller Tester 3.6">
              <p id="toolNameErrorMessage" class="error-message"></p>

              <input type="text" name="toolNote" v-model="toolNote" class="form-control" placeholder="Controller_Tester-3.6.2-V12.exe">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">Controller Tester 트러블슈팅 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/controller-tester-tools/form/" + idx,
          {},
      )
          .then((response) => {
            ...
            toolCategory.value = response.data.toolCategory;
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/controller-tester-tools/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/controller-tester-tools/" + idx,
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/controller-tester-tools/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("controllerTesterToolIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/controller-tester-tools/attached-file",
            formData, {
              headers: {
      ...
      localStorage.setItem("result", "/controller-tester-tool/update-success");
      router.push("/controller-tester-tool/read/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/controller-tester-tools/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/controller-tester-tool/list/" + misraCIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      title, priority, hashTags, toolCategory, toolName, toolNote, ideName, compilerName, compilerNote,
      priorityArray,
