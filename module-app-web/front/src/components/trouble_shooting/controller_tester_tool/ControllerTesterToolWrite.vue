<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="Controller Tester 트러블슈팅" :paths="['Controller Tester', 'Controller Tester 트러블슈팅 작성']" title=""/>
        ...
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            ...
            <td style="overflow: visible">
              ...
              <input type="text" name="toolNote" v-model="toolNote" class="form-control" placeholder="Controller_Tester-3.6.2-V12">
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/controller-tester-tools/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/controller-tester-tools/priority-list-write",
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/controller-tester-tools",
          {
            title: title.value,
          ...
          .then((response) => {
            controllerTesterIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("controllerTesterIdx", controllerTesterIdx);
        ...
        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/controller-tester-tools/attached-file",
            ...
            .catch((error) => {
              ...
              deletePost(controllerTesterIdx);
              isExit = true;
            })
            .then(() => {
              return isExit;
      ...
      localStorage.setItem("result", "/controller-tester-tool/write-success");
      router.push("/controller-tester-tool/read/" + controllerTesterIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      title, priority, hashTags, toolCategory, toolName, toolNote, ideName, compilerName, compilerNote, activeStatus,
      priorityArray,
