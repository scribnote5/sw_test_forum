<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="COVER 트러블슈팅" :paths="['COVER', 'COVER 트러블슈팅 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">COVER 트러블슈팅 정보</th>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="overflow: visible">
              <input type="text" name="toolName" v-model="toolName" class="form-control" placeholder="COVER 4 EE sp2">
              <p id="toolNameErrorMessage" class="error-message"></p>

              <input type="text" name="toolNote" v-model="toolNote" class="form-control" placeholder="tmpa v6.1">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">COVER 트러블슈팅 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cover-tools/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cover-tools/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cover-tools/" + idx,
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cover-tools/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("coverToolIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cover-tools/attached-file",
            formData, {
              headers: {
      ...
      localStorage.setItem("result", "/cover-tool/update-success");
      router.push("/cover-tool/read/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cover-tools/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/cover-tool/list/" + misraCIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
