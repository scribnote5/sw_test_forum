<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="COVER 트러블슈팅" :paths="['COVER', 'COVER 트러블슈팅 작성']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cover-tools/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cover-tools/priority-list-write",
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cover-tools",
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("coverToolIdx", coverToolIdx);
      ...
      localStorage.setItem("result", "/cover-tool/write-success");
      router.push("/cover-tool/read/" + coverToolIdx);
    }
    return {
      // variable
