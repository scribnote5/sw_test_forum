<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="STATIC 트러블슈팅" :paths="['STATIC', 'STATIC 트러블슈팅 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">STATIC 트러블슈팅 정보</th>
          </tr>
          <tr>
          ...
          <tr>
            <th colspan="2" class="sub-item-title">STATIC 트러블슈팅 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/static-tools/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/static-tools/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/static-tools/" + idx,
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/static-tools/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("staticToolIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/static-tools/attached-file",
            formData, {
              headers: {
      ...
      localStorage.setItem("result", "/static-tool/update-success");
      router.push("/static-tool/read/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/static-tools/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/static-tool/list/" + misraCIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
