<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="STATIC 트러블슈팅" :paths="['STATIC', 'STATIC 트러블슈팅 작성']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/static-tools/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/static-tools/priority-list-write",
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/static-tools",
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("staticToolIdx", staticToolIdx);
      ...
      localStorage.setItem("result", "/static-tool/write-success");
      router.push("/static-tool/read/" + staticToolIdx);
    }
    return {
      // variable
