<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page=".NET Framework Design Guideline 예제 코드" :subPage="fxCopRule" :paths="['.NET Framework Design Guideline', '.NET Framework Design Guideline 에제 코드 수정']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-examples/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-examples/priority-list-update/" + idx + "/" + fxCopIdx.value,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-examples/" + idx,
          {
      ...
      localStorage.setItem("result", "/fx-cop-example/update-success");
      router.push("/fx-cop-example/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-examples/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/fx-cop-example/list/" + fxCopIdx.value);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      fxCopIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
