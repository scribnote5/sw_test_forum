<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page=".NET Framework Design Guideline 가이드라인" :subPage="fxCopRule" :paths="['.NET Framework Design Guideline', '.NET Framework Design Guideline 가이드라인 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">FxCop 가이드라인 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
      ...
      <div class="d-flex justify-content-between mx-3 my-5">
        <div class="d-flex">
          <router-link :to="'/fx-cop-guideline/list/' + fxCopIdx">
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-guidelines/form/" + idx,
          {},
      )
          .then((response) => {
            fxCopIdx.value = response.data.fxCopIdx;
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-guidelines/" + idx,
          {
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-guidelines/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("fxCopGuidelineIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-guidelines/attached-file",
            formData, {
              headers: {
      ...
      localStorage.setItem("result", "/fx-cop-guideline/update-success");
      router.push("/fx-cop-guideline/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-guidelines/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/fx-cop-guideline/list/" + fxCopIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      fxCopIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
      fxCopRule,
