...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-guidelines/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/fx-cop-guidelines",
          {
          ...
          .then((response) => {
            fxCopGuidelineIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("fxCopGuidelineIdx", fxCopGuidelineIdx);
      ...
      localStorage.setItem("result", "/fx-cop-guideline/write-success");
      router.push("/fx-cop-guideline/read/from-rule-list/" + fxCopGuidelineIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      fxCopIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
      fxCopRule,
