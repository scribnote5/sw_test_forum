<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="사용 방법" :paths="['도구', '사용 방법 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">사용 방법 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/tool-usage-methods/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/tool-usage-methods/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/tool-usage-methods/" + idx,
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/tool-usage-methods/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("toolUsageMethodIdx", idx);
      ...
      localStorage.setItem("result", "/tool-usage-method/update-success");
      router.push("/tool-usage-method/read/" + idx);
    }
    ...
    return {
      ...
      title, priority, hashTags, toolCategory, targetToolName,
      priorityArray,
