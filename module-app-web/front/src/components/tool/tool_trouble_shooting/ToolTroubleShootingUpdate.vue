<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="트러블슈팅" :paths="['도구', '트러블슈팅 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">트러블슈팅 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/tool-trouble-shootings/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/tool-trouble-shootings/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/tool-trouble-shootings/" + idx,
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/tool-trouble-shootings/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("toolTroubleShootingIdx", idx);
      ...
      localStorage.setItem("result", "/tool-trouble-shooting/update-success");
      router.push("/tool-trouble-shooting/read/" + idx);
    }
    ...
    return {
      ...
      title, priority, hashTags, toolCategory, targetToolName, toolName, toolNote, ideName, compilerName, compilerNote,
      priorityArray,
