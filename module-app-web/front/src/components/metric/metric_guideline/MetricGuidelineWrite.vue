<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="메트릭 가이드라인" :subPage="metricRule" :paths="['메트릭', '메트릭 가이드라인 작성']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // variable
    const metricIdx = ref(route.params.metricIdx);
    const title = ref("");
    const hashTags = ref("");
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-guidelines/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-guidelines",
          {
          ...
          .then((response) => {
            metricGuidelineIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("metricGuidelineIdx", metricGuidelineIdx);
      ...
      localStorage.setItem("result", "/metric-guideline/write-success");
      router.push("/metric-guideline/read/from-rule-list/" + metricGuidelineIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      metricIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
      metricRule,
