<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="메트릭 가이드라인" :subPage="metricRule" :paths="['메트릭', '메트릭 가이드라인 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">메트릭 가이드라인 정보</th>
          </tr>
          <tr>
          ...
          <tr>
            <th colspan="2" class="sub-item-title">메트릭 가이드라인 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-guidelines/form/" + idx,
          {},
      )
          .then((response) => {
            metricIdx.value = response.data.metricIdx;
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric/metric-rule/" + metricIdx.value,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-guidelines/" + idx,
          {
            metricIdx: metricIdx.value,
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-guidelines/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("metricGuidelineIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-guidelines/attached-file",
            formData, {
              headers: {
      ...
      localStorage.setItem("result", "/metric-guideline/update-success");
      router.push("/metric-guideline/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-guidelines/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/metric-guideline/list/" + metricIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      metricIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
      metricRule,
