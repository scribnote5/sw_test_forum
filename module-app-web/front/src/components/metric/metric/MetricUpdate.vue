<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="메트릭 규칙" :paths="['메트릭', '메트릭 규칙 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            ...
            <td style="overflow: visible">
              <div class="autoComplete_wrapper">
                <input type="text" name="title" v-model="title" class="form-control" placeholder="함수 내 분기문 개수(Cyclomatic Complexity, FUCYC)">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">메트릭 규칙 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric/" + idx,
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric/attached-file/", {
      ...
      localStorage.setItem("result", "/metric/update-success");
      router.push("/metric/read/" + idx);
    }
