<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="메트릭 예제 코드" :subPage="metricRule" :paths="['메트릭', '메트릭 에제 코드 수정']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-examples/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-examples/priority-list-update/" + idx + "/" + metricIdx.value,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-examples/" + idx,
          {
      ...
      localStorage.setItem("result", "/metric-example/update-success");
      router.push("/metric-example/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-examples/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/metric-example/list/" + metricIdx.value);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      metricIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
