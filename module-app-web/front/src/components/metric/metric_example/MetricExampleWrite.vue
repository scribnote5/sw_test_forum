<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="메트릭 예제 코드" :subPage="metricRule" :paths="['메트릭', '메트릭 예제 코드 작성']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">메트릭 예제 코드 정보</th>
          </tr>
          <tr>
      ...
      <div class="d-flex justify-content-between mx-4 my-5">
        <div class="d-flex">
          <router-link :to="'/metric-example/list/' + metricIdx">
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-examples/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
          ...
          .catch((error) => {
            ...
            router.push("/metric-example/list/");
          })
          .then(() => {
          });

      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-examples/priority-list-write/" + metricIdx.value,
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/metric-examples",
          {
          ...
          .then((response) => {
            metricExampleIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      localStorage.setItem("result", "/metric-example/write-success");
      router.push("/metric-example/read/from-rule-list/" + metricExampleIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      metricIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
      metricRule, priorityArray,
