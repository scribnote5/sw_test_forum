<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="Java Code Conventions 규칙" :paths="['Java Code Conventions', 'Java Code Conventions 규칙 작성']" title=""/>
          ...
          <tr>
            ...
            <td style="overflow: visible">
              <div class="autoComplete_wrapper">
                <input type="text" name="title" id="title" v-model="title" class="form-control" placeholder="메소드 선언은 분리 되어야 함">
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions/priority-list-write",
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions",
          {
            title: title.value,
          ...
          .then((response) => {
            javaCodeConventionsIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("javaCodeConventionsIdx", javaCodeConventionsIdx);
        ...
        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions/attached-file",
            ...
            .catch((error) => {
              ...
              deletePost(javaCodeConventionsIdx);
              isExit = true;
            })
            .then(() => {
              return isExit;
      ...
      localStorage.setItem("result", "/java-code-conventions/write-success");
      router.push("/java-code-conventions/read/" + javaCodeConventionsIdx);
    }
    return {
      // variable
