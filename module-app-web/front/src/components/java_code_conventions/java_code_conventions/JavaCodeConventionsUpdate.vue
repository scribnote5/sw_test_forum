<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="Java Code Conventions 규칙" :paths="['Java Code Conventions', 'Java Code Conventions 규칙 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">Java Code Conventions 규칙 정보</th>
          </tr>
          <tr>
            <th>제목<span class="required-field">*</span></th>
            <td style="overflow: visible">
              <div class="autoComplete_wrapper">
                <input type="text" name="title" v-model="title" class="form-control" placeholder="메소드 선언은 분리 되어야 함">
          ...
          <tr>
            ...
            <td>
              <input type="text" name="originalTitle" v-model="originalTitle" class="form-control" placeholder="Method declaration must be separated">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">Java Code Conventions 표기 방식</th>
          </tr>
          <tr>
            <th>STATIC 규칙<span class="recommended-field">*</span></th>
            <td>
              <input type="text" name="staticTitle" v-model="staticTitle" class="form-control" placeholder="Java Code Conventions 규칙과 매핑되는 STATIC 규칙을 작성해주세요.">
          ...
          <tr>
            ...
            <td>
              <input type="text" name="sparrowTitle" v-model="sparrowTitle" class="form-control" placeholder="Java Code Conventions 규칙과 매핑되는 Sparrow 규칙을 작성해주세요.">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">Java Code Conventions 규칙 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      if (!(validateLengthAndIsEmpty("title", title.value)
          ...
          && validateLength("sparrowTitle", sparrowTitle.value)
      )) {
        return false;
      }

      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions/" + idx,
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("javaCodeConventionsIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions/attached-file",
            formData, {
              headers: {
      ...
      localStorage.setItem("result", "/java-code-conventions/update-success");
      router.push("/java-code-conventions/read/" + idx);
    }
