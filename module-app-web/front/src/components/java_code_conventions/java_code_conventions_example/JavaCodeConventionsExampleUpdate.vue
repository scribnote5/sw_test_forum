<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="Java Code Conventions 예제 코드" :subPage="javaCodeConventionsRule" :paths="['Java Code Conventions', 'Java Code Conventions 에제 코드 수정']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions-examples/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions-examples/priority-list-update/" + idx + "/" + javaCodeConventionsIdx.value,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions-examples/" + idx,
          {
      ...
      localStorage.setItem("result", "/java-code-conventions-example/update-success");
      router.push("/java-code-conventions-example/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions-examples/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/java-code-conventions-example/list/" + javaCodeConventionsIdx.value);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      javaCodeConventionsIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
