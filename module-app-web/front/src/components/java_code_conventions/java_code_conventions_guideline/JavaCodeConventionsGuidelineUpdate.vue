<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="Java Code Conventions 가이드라인" :subPage="javaCodeConventionsRule" :paths="['Java Code Conventions', 'Java Code Conventions 가이드라인 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            <th colspan="2" class="sub-item-title">Java Code Conventions 가이드라인 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // variable
    let javaCodeConventionsIdx = ref("");
    let title = ref("");
    let hashTags = ref("");
    let hashTagsIdx = ref("");
    let projectName = ref("");
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions-guidelines/form/" + idx,
          {},
      )
          .then((response) => {
            javaCodeConventionsIdx.value = response.data.javaCodeConventionsIdx;
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions-guidelines/" + idx,
          {
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions-guidelines/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("javaCodeConventionsGuidelineIdx", idx);
      ...
      localStorage.setItem("result", "/java-code-conventions-guideline/update-success");
      router.push("/java-code-conventions-guideline/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/java-code-conventions-guidelines/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/java-code-conventions-guideline/list/" + javaCodeConventionsIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      javaCodeConventionsIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
