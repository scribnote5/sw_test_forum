      ...
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            ...
            <td style="overflow: visible">
              <div class="autoComplete_wrapper">
                <input type="text" name="title" id="title" v-model="title" class="form-control" placeholder="[00_01_01] 미달성 코드(unreachable code) 제거">
          ...
          <tr>
            ...
            <td>
              <input type="text" name="originalTitle" v-model="originalTitle" class="form-control" placeholder="[00_01_01] A project shall not contain unreachable code">
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp/" + idx,
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("misraCppIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp/attached-file",
            formData, {
              headers: {
      ...
      localStorage.setItem("result", "/misra-cpp/update-success");
      router.push("/misra-cpp/read/" + idx);
    }
    ...
    return {
      ...
      title, originalTitle, priority, frequency, hashTags, category, qacTitle,
      priorityArray,
