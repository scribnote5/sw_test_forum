...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp/priority-list-write",
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp",
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("misraCppIdx", misraCppIdx);
      ...
      localStorage.setItem("result", "/misra-cpp/write-success");
      router.push("/misra-cpp/read/" + misraCppIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      title, originalTitle, priority, frequency, hashTags, category, qacTitle, activeStatus,
      priorityArray,
