<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="MISRA C++ 가이드라인" :subPage="misraCppRule" :paths="['MISRA C++', 'MISRA C++ 가이드라인 작성']" title=""/>
        ...
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            ...
            <td>
              ...
              <input type="text" name="guidelineResultNote" v-model="guidelineResultNote" class="form-control" placeholder="STATIC 4.3.2에 패치 예정">
...
export default {
  ...
  setup() {
    ...
    // variable
    const misraCppIdx = ref(route.params.misraCppIdx);
    const title = ref("");
    const hashTags = ref("");
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-guidelines/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-guidelines",
          {
          ...
          .then((response) => {
            misraCppGuidelineIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("misraCppGuidelineIdx", misraCppGuidelineIdx);
        ...
        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-guidelines/attached-file",
            ...
            .catch((error) => {
              ...
              deletePost(misraCppGuidelineIdx);
              isExit = true;
            })
            .then(() => {
              return isExit;
      ...
      localStorage.setItem("result", "/misra-cpp-guideline/write-success");
      router.push("/misra-cpp-guideline/read/from-rule-list/" + misraCppGuidelineIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      misraCppIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
      misraCppRule,
