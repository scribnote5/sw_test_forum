<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="MISRA C++ 예제 코드" :subPage="misraCppRule" :paths="['MISRA C++', 'MISRA C++ 예제 코드 리스트']" title=""/>
    ...
    <div class="container-fluid">
        ...
        <table class="table table-hover mobile-table-list mt-4">
          ...
          <tr v-for="(misraCppExample, i) in misraCppExampleList.content" :key="i">
            ...
            <td>
              ...
              <span class="comment-count">{{ misraCppExample.commentDtoCount }}</span>
              <img v-if="misraCppExample.newIcon" :src="require(`@/assets/images/new_post.svg`)" class="new-icon"/>
              <!-- Mobile -->
              <div class="d-inline d-lg-none">
                <div>
                  <span class="mobile-content">{{ misraCppExample.createdByUser.department }} {{ misraCppExample.createdByUser.name }} </span> <br>
                  <span class="mobile-content">{{ misraCppExample.createdDate }}</span> &nbsp;
                  <span class="mobile-content"> 조회수: {{ misraCppExample.views }}</span> &nbsp;
                </div>
              </div>
            </td>
            <!-- Desktop -->
            <td class="d-none d-lg-table-cell">{{ misraCppExample.createdByUser.department }} {{ misraCppExample.createdByUser.name }}</td>
            <td class="d-none d-lg-table-cell text-center">{{ misraCppExample.createdDate }}</td>
            <td class="d-none d-lg-table-cell text-center">{{ misraCppExample.views }}</td>
          </tr>

          <tr v-if="misraCppExampleList.content.length === 0">
      ...
      <!-- Pagination -->
      <Pagination :first="misraCppExampleList.first" :last="misraCppExampleList.last" :totalPages="misraCppExampleList.totalPages"
                  :number="misraCppExampleList.number" :startNumber="startNumber" :endNumber="endNumber"
      ...
      <div v-if="misraCppIdx > 0" class="d-flex justify-content-between mx-4 mb-4">
        ...
        <router-link :to="'/misra-cpp-example/write/' + misraCppIdx" v-if="access">
...
export default {
  ...
  setup() {
    ...
    let misraCppExampleList = ref({content: {}});
    ...
    onBeforeMount(async () => {
      ...
      // 리스트만 검색 정보 불러오기 가능
      if (misraCppIdx == 0 && store.state.pageInfo.pageName === "MisraCppExampleList") {
    ...
    const searchList = async (pageParam) => {
      ...
      const uri = createUri(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-examples/list", params);

      await axios.get(uri,
          {},
      )
          .then((response) => {
            // misraCppExampleList.value.content.length = 0;
            misraCppExampleList.value = response.data;
            startNumber.value = Math.floor(misraCppExampleList.value.number / 10) * 10 + 1;
            endNumber.value = (misraCppExampleList.value.totalPages > startNumber.value + 9) ? startNumber.value + 9 : (misraCppExampleList.value.totalPages == 0 ? 1 : misraCppExampleList.value.totalPages);

            // dayjs
            for (const misraCppExample of misraCppExampleList.value.content) {
              misraCppExample.createdDate = dayjs(misraCppExample.createdDate).format("YYYY.MM.DD.");
            }
          })
          .catch((error) => {
            parseApiErrorMsg(error.response);
    ...
    return {
      // variable
      misraCppIdx, misraCppExampleListByPriority, misraCppExampleList, searchType, searchKeyword, startNumber, endNumber, access,
      misraCppRule,
