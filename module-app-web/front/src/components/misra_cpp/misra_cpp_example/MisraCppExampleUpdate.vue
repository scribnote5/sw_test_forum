<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="MISRA C++ 예제 코드" :subPage="misraCppRule" :paths="['MISRA C++', 'MISRA C++ 에제 코드 수정']" title=""/>
    ...
    <div class="container-fluid">
      ...
      <div class="d-flex justify-content-between mx-4 my-5">
        <div class="d-flex">
          <router-link :to="'/misra-cpp-example/list/' + misraCppIdx">
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-examples/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-examples/priority-list-update/" + idx + "/" + misraCppIdx.value,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-examples/" + idx,
          {
      ...
      localStorage.setItem("result", "/misra-cpp-example/update-success");
      router.push("/misra-cpp-example/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-examples/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/misra-cpp-example/list/" + misraCppIdx.value);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      misraCppIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
      misraCppRule, priorityArray,
