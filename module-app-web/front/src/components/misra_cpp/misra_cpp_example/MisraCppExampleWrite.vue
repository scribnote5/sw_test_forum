<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="MISRA C++ 예제 코드" :subPage="misraCppRule" :paths="['MISRA C++', 'MISRA C++ 예제 코드 작성']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-examples/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
          ...
          .catch((error) => {
            ...
            router.push("/misra-cpp-example/list/");
          })
          .then(() => {
          });

      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-examples/priority-list-write/" + misraCppIdx.value,
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-cpp-examples",
          {
          ...
          .then((response) => {
            misraCppExampleIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      localStorage.setItem("result", "/misra-cpp-example/write-success");
      router.push("/misra-cpp-example/read/from-rule-list/" + misraCppExampleIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      misraCppIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
