<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="동적시험 미달성 코드 분석" :paths="['나머지 자료', '동적시험 미달성 코드 분석 작성']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">동적시험 미달성 코드 분석 정보</th>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>
              ...
              <input type="text" name="guidelineResultNote" v-model="guidelineResultNote" class="form-control" placeholder="방어 코드 수정">
          ...
          <tr>
            ...
            <td style="overflow: visible">
              <input type="text" name="toolName" v-model="toolName" class="form-control" placeholder="COVER 4 SP 3">
              <p id="toolNameErrorMessage" class="error-message"></p>

              <input type="text" name="toolNote" v-model="toolNote" class="form-control" placeholder="">
          ...
          <tr>
            <td colspan="2">
              <CodeMirror pageInformation="write" mode="text/x-c++src"></CodeMirror>
            </td>
          </tr>
          <tr>
            <td colspan="2">
...
import {editor, editorConfig, editorDynamicTestData} from "@/assets/plugins/ckeditor/ckeditor-init"
...
export default {
  ...
  setup() {
    ...
    const vueEditorData = editorDynamicTestData;
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/dynamic-tests/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/dynamic-tests",
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("dynamicTestIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/dynamic-tests/attached-file",
            formData, {
              headers: {
                "Content-Type": "multipart/forXm-data"
              }
            }
        )
            .then((response) => {
      ...
      localStorage.setItem("result", "/dynamic-test/write-success");
      router.push("/dynamic-test/read/" + idx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
