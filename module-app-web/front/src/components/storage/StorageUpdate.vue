<template>
  <section>
    ...
    <Breadcrumb page="지식 저장소" :paths="['관리자 페이지', '지식 저장소 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">지식 저장소 정보</th>
          </tr>
          <tr>
          ...
          <tr>
            <th colspan="2" class="sub-item-title">지식 저장소 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/storages/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/storages/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/storages/" + idx,
          {
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/storages/attached-file", {
      ...
      localStorage.setItem("result", "/storage/update-success");
