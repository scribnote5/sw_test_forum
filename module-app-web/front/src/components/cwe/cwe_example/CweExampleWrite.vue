<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="CWE C/C++ 예제 코드" :subPage="cweRule" :paths="['CWE C/C++', 'CWE C/C++ 예제 코드 작성']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">CWE C/C++ 예제 코드 정보</th>
          </tr>
          <tr>
      ...
      <div class="d-flex justify-content-between mx-3 my-5">
        <div class="d-flex">
          <router-link :to="'/cwe-example/list/' + cweIdx">
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-examples/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
          ...
          .catch((error) => {
            ...
            router.push("/cwe-example/list/");
          })
          .then(() => {
          });

      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-examples/priority-list-write/" + cweIdx.value,
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-examples",
          {
            cweIdx: cweIdx.value,
            title: title.value,
      ...
      localStorage.setItem("result", "/cwe-example/write-success");
      router.push("/cwe-example/read/from-rule-list/" + cweExampleIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      cweIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
      cweRule, priorityArray,
