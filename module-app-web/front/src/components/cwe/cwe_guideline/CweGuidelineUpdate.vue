<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="CWE C/C++ 가이드라인" :subPage="cweRule" :paths="['CWE C/C++', 'CWE C/C++ 가이드라인 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">CWE C/C++ 가이드라인 정보</th>
          </tr>
          <tr>
          ...
          <tr>
            <th colspan="2" class="sub-item-title">CWE C/C++ 가이드라인 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-guidelines/form/" + idx,
          {},
      )
          .then((response) => {
            cweIdx.value = response.data.cweIdx;
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe/cwe-rule/" + cweIdx.value,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-guidelines/" + idx,
          {
            cweIdx: cweIdx.value,
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-guidelines/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("cweGuidelineIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-guidelines/attached-file",
            formData, {
              headers: {
      ...
      localStorage.setItem("result", "/cwe-guideline/update-success");
      router.push("/cwe-guideline/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-guidelines/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/cwe-guideline/list/" + cweIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      cweIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
      cweRule,
