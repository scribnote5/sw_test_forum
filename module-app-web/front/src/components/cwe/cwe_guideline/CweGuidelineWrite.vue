<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="CWE C/C++ 가이드라인" :subPage="cweRule" :paths="['CWE C/C++', 'CWE C/C++ 가이드라인 작성']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // variable
    const cweIdx = ref(route.params.cweIdx);
    const title = ref("");
    const hashTags = ref("");
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-guidelines/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-guidelines",
          {
            cweIdx: cweIdx.value,
            title: title.value,
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("cweGuidelineIdx", cweGuidelineIdx);
      ...
      localStorage.setItem("result", "/cwe-guideline/write-success");
      router.push("/cwe-guideline/read/from-rule-list/" + cweGuidelineIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      cweIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
      cweRule,
