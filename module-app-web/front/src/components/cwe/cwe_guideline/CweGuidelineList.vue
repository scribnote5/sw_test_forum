<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="CWE C/C++ 가이드라인" :subPage="cweRule" :paths="['CWE C/C++', 'CWE C/C++ 가이드라인 리스트']" title=""/>
    ...
    <div class="container-fluid">
      ...
      <div class="page-content">
        <table class="table table-hover mobile-table-list mt-4">
          ...
          <tr v-for="(cweGuideline, i) in cweGuidelineList.content" :key="i">
            ...
            <td>
              ...
              <div class="d-inline d-lg-none">
                <div>
                  <span class="mobile-content">{{ cweGuideline.createdByUser.department }} {{ cweGuideline.createdByUser.name }} </span> <br>
                  ...
                  <span class="mobile-content" v-if="cweGuideline.guidelineResult == 'COMPLETED'">수정 완료</span>
                  <span class="mobile-content" v-if="cweGuideline.guidelineResult == 'EXCLUDE'">사전 제외</span>
                  <span class="mobile-content" v-if="cweGuideline.guidelineResult == 'EXCEPTION'">예외 처리</span>
                  <span class="mobile-content" v-if="cweGuideline.guidelineResult == 'FALSE_ALARM'">도구 오탐</span>
                  <span class="mobile-content" v-if="cweGuideline.guidelineResult == 'FALSE_ALARM_PATCHED'">도구 오탐 패치 완료</span>
                </div>
              </div>
            </td>
            <!-- Desktop -->
            <td class="d-none d-lg-table-cell">{{ cweGuideline.createdByUser.department }} {{ cweGuideline.createdByUser.name }}</td>
            ...
            <td class="d-none d-lg-table-cell text-center">
              ...
              <span v-if="cweGuideline.guidelineResult == 'FALSE_ALARM_PATCHED'">도구 오탐 패치 완료</span>
            </td>
          ...
          <tr v-if="cweGuidelineList.content.length === 0">
      ...
      <!-- Pagination -->
      <Pagination :first="cweGuidelineList.first" :last="cweGuidelineList.last" :totalPages="cweGuidelineList.totalPages"
                  :number="cweGuidelineList.number" :startNumber="startNumber" :endNumber="endNumber"
      ...
      <div v-if="cweIdx > 0" class="d-flex justify-content-between mx-4 mb-4">
        ...
        <router-link :to="'/cwe-guideline/write/' + cweIdx" v-if="access">
...
export default {
  ...
  setup() {
    ...
    // variable
    let cweGuidelineList = ref({content: {}});
    ...
    onBeforeMount(async () => {
      ...
      if (cweIdx > 0) {
        ...
        cweRule.value = "CWE C/C++ 규칙 페이지에서만 가이드라인을 등록할 수 있습니다.";
      }

      // 검색 정보 불러오기
      // 리스트와 규칙 idx로 조회하는 리스트는 서로 다른 검색 정보를 가져야 함
      // 리스트만 검색 정보 불러오기 가능
      if (cweIdx == 0 && store.state.pageInfo.pageName === "CweGuidelineList") {
      ...
      // 페이지 정보 불러오기
      if (cweIdx == 0 && store.state.pageInfo.pageName === "CweGuidelineList") {
        cweGuidelineList.value.number = cweGuidelineList.value.number < endNumber.value ? store.state.pageInfo.page : 0;
      }

      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-guidelines/list-access-authority",
          {},
      )
          .then((response) => {
    ...
    const searchList = async (pageParam) => {
      ...
      const uri = createUri(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-guidelines/list", params);

      await axios.get(uri,
          {},
      )
          .then((response) => {
            // cweGuidelineList.value.content.length = 0;
            cweGuidelineList.value = response.data;
            startNumber.value = Math.floor(cweGuidelineList.value.number / 10) * 10 + 1;
            endNumber.value = (cweGuidelineList.value.totalPages > startNumber.value + 9) ? startNumber.value + 9 : (cweGuidelineList.value.totalPages == 0 ? 1 : cweGuidelineList.value.totalPages);

            // dayjs
            for (const cweGuideline of cweGuidelineList.value.content) {
        ...
        store.commit("pageInfo/setPageInfo",
            {
              ...
              page: cweGuidelineList.value.number,
              pageName: 'CweGuidelineList'
            }
        );
      }
    });
    ...
    return {
      // variable
      cweIdx, cweGuidelineList, searchType, searchKeyword, startNumber, endNumber, access,
      cweRule,
