...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe/priority-list-update/" + idx,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe/attached-file/", {
      ...
      localStorage.setItem("result", "/cwe/update-success");
      router.push("/cwe/read/" + idx);
    }
