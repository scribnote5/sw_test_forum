<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="MISRA C 가이드라인" :subPage="misraCRule" :paths="['MISRA C', 'MISRA C 가이드라인 수정']" title=""/>
      ...
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tr>
            ...
            <td style="overflow: visible">
              <input type="text" name="toolName" v-model="toolName" class="form-control">
          ...
          <tr>
            ...
            <td style="overflow: visible">
              <input type="text" name="compilerName" v-model="compilerName" class="form-control">
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-guidelines/form/" + idx,
          {},
      )
          .then((response) => {
            misraCIdx.value = response.data.misraCIdx;
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c/misra-c-rule/" + misraCIdx.value,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-guidelines/" + idx,
          {
            misraCIdx: misraCIdx.value,
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-guidelines/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("misraCGuidelineIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-guidelines/attached-file",
            formData, {
              headers: {
      ...
      localStorage.setItem("result", "/misra-c-guideline/update-success");
      router.push("/misra-c-guideline/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-guidelines/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/misra-c-guideline/list/" + misraCIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      misraCIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
      misraCRule,
