<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="MISRA C 가이드라인" :subPage="misraCRule" :paths="['MISRA C', 'MISRA C 가이드라인 작성']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // variable
    const misraCIdx = ref(route.params.misraCIdx);
    const title = ref("");
    const hashTags = ref("");
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-guidelines/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-guidelines",
          {
          ...
          .then((response) => {
            misraCGuidelineIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("misraCGuidelineIdx", misraCGuidelineIdx);
      ...
      localStorage.setItem("result", "/misra-c-guideline/write-success");
      router.push("/misra-c-guideline/read/from-rule-list/" + misraCGuidelineIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      misraCIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
      misraCRule,
