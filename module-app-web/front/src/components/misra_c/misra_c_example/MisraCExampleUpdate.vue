<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="MISRA C 예제 코드" :subPage="misraCRule" :paths="['MISRA C', 'MISRA C 에제 코드 수정']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-examples/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-examples/priority-list-update/" + idx + "/" + misraCIdx.value,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-examples/" + idx,
          {
      ...
      localStorage.setItem("result", "/misra-c-example/update-success");
      router.push("/misra-c-example/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-examples/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/misra-c-example/list/" + misraCIdx.value);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      misraCIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
