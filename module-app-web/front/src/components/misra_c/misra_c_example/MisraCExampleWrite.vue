<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="MISRA C 예제 코드" :subPage="misraCRule" :paths="['MISRA C', 'MISRA C 예제 코드 작성']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">MISRA C 예제 코드 정보</th>
          </tr>
          <tr>
      ...
      <div class="d-flex justify-content-between mx-3 my-5">
        <div class="d-flex">
          <router-link :to="'/misra-c-example/list/' + misraCIdx">
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-examples/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
          ...
          .catch((error) => {
            ...
            router.push("/misra-c-example/list/");
          })
          .then(() => {
          });

      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-examples/priority-list-write/" + misraCIdx.value,
          {},
      )
          .then((response) => {
    ...
    const deletePost = (idx) => {
      axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-examples/" + idx,
          {},
      )
          .then((response) => {
          })
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/misra-c-examples",
          {
          ...
          .then((response) => {
            misraCExampleIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      localStorage.setItem("result", "/misra-c-example/write-success");
      router.push("/misra-c-example/read/from-rule-list/" + misraCExampleIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      misraCIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
      misraCRule, priorityArray,
