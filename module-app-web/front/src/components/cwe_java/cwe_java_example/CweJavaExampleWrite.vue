<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="CWE Java 예제 코드" :subPage="cweJavaRule" :paths="['CWE Java', 'CWE Java 예제 코드 작성']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-examples/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
          ...
          .catch((error) => {
            ...
            router.push("/cwe-java-example/list/");
          })
          .then(() => {
          });

      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-examples/priority-list-write/" + cweJavaIdx.value,
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-examples",
          {
          ...
          .then((response) => {
            cweJavaExampleIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      localStorage.setItem("result", "/cwe-java-example/write-success");
      router.push("/cwe-java-example/read/from-rule-list/" + cweJavaExampleIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      cweJavaIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
