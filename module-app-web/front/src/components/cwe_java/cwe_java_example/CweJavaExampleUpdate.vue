<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="CWE Java 예제 코드" :subPage="cweJavaRule" :paths="['CWE Java', 'CWE Java 에제 코드 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">CWE Java 예제 코드 정보</th>
          </tr>
          <tr>
      ...
      <div class="d-flex justify-content-between mx-3 my-5">
        <div class="d-flex">
          <router-link :to="'/cwe-java-example/list/' + cweJavaIdx">
...
// utils
import {parseErrorMsg, validateEditor, validateLength, validateLengthAndIsEmpty, validateSize} from "@/utils/validation-util";
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-examples/form/" + idx,
          {},
      )
          .then((response) => {
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-examples/priority-list-update/" + idx + "/" + cweJavaIdx.value,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-examples/" + idx,
          {
      ...
      localStorage.setItem("result", "/cwe-java-example/update-success");
      router.push("/cwe-java-example/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-examples/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/cwe-java-example/list/" + cweJavaIdx.value);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      cweJavaIdx, title, priority, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
      cweJavaRule, priorityArray,
