<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="CWE Java 가이드라인" :subPage="cweJavaRule" :paths="['CWE Java', 'CWE Java 가이드라인 작성']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // variable
    const cweJavaIdx = ref(route.params.cweJavaIdx);
    const title = ref("");
    const hashTags = ref("");
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-guidelines/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
    ...
    const writePost = async (vueEditorData) => {
      ...
      isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-guidelines",
          {
          ...
          .then((response) => {
            cweJavaGuidelineIdx = response.data;
            isExit = false;
          })
          .catch((error) => {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("cweJavaGuidelineIdx", cweJavaGuidelineIdx);
        ...
        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-guidelines/attached-file",
            ...
            .catch((error) => {
              ...
              deletePost(cweJavaGuidelineIdx);
              isExit = true;
            })
            .then(() => {
              return isExit;
      ...
      localStorage.setItem("result", "/cwe-java-guideline/write-success");
      router.push("/cwe-java-guideline/read/from-rule-list/" + cweJavaGuidelineIdx);
    }
    return {
      // variable
      vueEditor, vueEditorData, vueEditorConfig,
      cweJavaIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, activeStatus,
      cweJavaRule,
