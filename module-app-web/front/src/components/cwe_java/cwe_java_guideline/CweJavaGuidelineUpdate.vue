<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="CWE Java 가이드라인" :subPage="cweJavaRule" :paths="['CWE Java', 'CWE Java 가이드라인 수정']" title=""/>
    ...
    <div class="container-fluid">
      <div class="page-content">
        <table class="table mobile-table-read mt-4">
          ...
          <tbody>
          <tr>
            <th colspan="2" class="sub-item-title">CWE Java 가이드라인 정보</th>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="overflow: visible">
              <input type="text" name="projectName" v-model="projectName" class="form-control" placeholder="한화시스템 통합랩 - FFX B3 MFR 신호처리">
          ...
          <tr>
            ...
            <td>
              <select v-model="guidelineResult" class="form-select">
                ...
                <option value="EXCEPTION">예외 처리(사유서 처리)</option>
                <option value="FALSE_ALARM">도구 오탐(False Alarm)</option>
                <option value="FALSE_ALARM_PATCHED">도구 오탐(False Alarm) 패치 완료</option>
              </select>
          ...
          <tr>
            <th colspan="2" class="sub-item-title">CWE Java 가이드라인 부가정보</th>
          </tr>
          <tr>
            <th>작성자</th>
      ...
      <div class="d-flex justify-content-between mx-3 my-5">
        <div class="d-flex">
          <router-link :to="'/cwe-java-guideline/list/' + cweJavaIdx">
...
// utils
import {deleteArrayIndexIsEmpty, parseErrorMsg, validateEditor, validateLength, validateLengthAndIsEmpty, validateSize} from "@/utils/validation-util";
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-guidelines/form/" + idx,
          {},
      )
          .then((response) => {
            cweJavaIdx.value = response.data.cweJavaIdx;
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java/cwe-java-rule/" + cweJavaIdx.value,
          {},
      )
          .then((response) => {
    ...
    const updatePost = async () => {
      ...
      if (!(validateLengthAndIsEmpty("title", title.value)
          ...
          && validateLength("projectName", projectName.value)
          && validateLength("guidelineResultNote", guidelineResultNote.value)
          && validateLength("toolName", toolName.value)
          && validateLength("toolNote", toolNote.value)
          && validateLength("compilerName", compilerName.value)
          && validateLength("compilerNote", compilerNote.value)
          && validateSize("nonCompliantExample", nonCompliantExample.value)
          && validateSize("compliantExample", compliantExample.value)
      )) {
        return false;
      }

      axios.put(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-guidelines/" + idx,
          {
            cweJavaIdx: cweJavaIdx.value,
            title: title.value,
      ...
      if (!isEmpty(fileUpload.deleteFileArray)) {
        isExit = await axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-guidelines/attached-file/", {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("cweJavaGuidelineIdx", idx);

        isExit = await axios.post(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-guidelines/attached-file",
            formData, {
              headers: {
            ...
            .catch((error) => {
              ...
              document.getElementById("totalFileSize").innerHTML = convertFileSize(oriTotalFileSize);
              isExit = true;
            })
            .then(() => {
              return isExit;
      ...
      localStorage.setItem("result", "/cwe-java-guideline/update-success");
      router.push("/cwe-java-guideline/read/from-rule-list/" + idx);
    }
        ...
        if (result.isConfirmed) {
          axios.delete(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java-guidelines/" + idx,
              ...
              .then((response) => {
                ...
                router.push("/cwe-java-guideline/list/" + cweJavaIdx);
              })
              .catch((error) => {
                parseErrorMsg(error.response);
              })
    ...
    return {
      ...
      cweJavaIdx, title, hashTags, projectName, guidelineResult, guidelineResultNote, toolName, toolNote, compilerName, compilerNote, nonCompliantExample, compliantExample, badCasePositionList, goodCasePositionList,
      cweJavaRule,
