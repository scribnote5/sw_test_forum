<template>
  <section>
    ...
    <!-- Breadcrumb -->
    <Breadcrumb page="CWE Java 규칙" :paths="['CWE Java', 'CWE Java 규칙 작성']" title=""/>
...
export default {
  ...
  setup() {
    ...
    // onBeforeMount, init
    onBeforeMount(async () => {
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java/form/0",
          {},
      )
          .then((response) => {
            // autoComplete 설정
      ...
      await axios.get(process.env.VUE_APP_MODULE_APP_API_URL + "/api/cwe-java/priority-list-write",
          {},
      )
          .then((response) => {
    ...
    const writePost = async (vueEditorData) => {
      ...
      if (!isEmpty(fileUpload.insertFileArray)) {
        ...
        formData.append("cweJavaIdx", cweJavaIdx);
      ...
      localStorage.setItem("result", "/cwe-java/write-success");
      router.push("/cwe-java/read/" + cweJavaIdx);
    }
    return {
      // variable
